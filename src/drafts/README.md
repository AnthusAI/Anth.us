...
Files here are **not** sourced by Gatsby (`gatsby-config.mjs` sources `src/site-content`
only), so they produce no page and no URL.

They were moved here because `state: draft` does not prevent publication —
`createPages` in `gatsby-node.js` builds a page for every MDX under `src/site-content`,
...
```bash
git mv src/drafts/<slug>.mdx src/site-content/<slug>.mdx
```

Articles under `src/site-content` with `state` other than `published` now render
