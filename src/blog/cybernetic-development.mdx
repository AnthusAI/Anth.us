...
{/* VISUAL PROMPT — cybernetic-development.png (HERO) --------------------
    NOT a stencil poster. The house character is the recurring white robot
    with glowing magenta eyes (cf. give-an-agent-a-tool.png, the homepage
    hero, the homepage capability grid). Slogan posters carry no information
    in a feed; a concept scene does, and it is warmer.

    The scene writes itself from the article's own etymology: cybernetics is
    from the Greek kybernetes, "steersman." So: a ship's wheel, with BOTH a
    human hand and the robot's hand on it. Not the robot steering alone (that
    is the glass-cockpit failure, which gets its own image later) and not the
    human alone. Both hands, same wheel. That is the Engine and the Governor
    in one frame, and it is the whole thesis.

    The current file at this path is a 6KB text-on-white placeholder, and it
    is also the preview_image -- the social card anywhere this is shared.
    Replacing it is the highest-value visual on the page.

    PROMPT:
    Photorealistic close shot of a polished wooden ship's wheel at sea. A
    human hand grips one spoke; the white ceramic hand of a small robot with
    glowing magenta eyes grips the adjacent spoke. Both are steering
    together. Warm low sun, soft ocean bokeh behind, shallow depth of field,
    the wheel and both hands sharp. Calm and purposeful, not tense. Square,
    2048x2048.

    Alt: "A human hand and a robot's hand steering a ship's wheel together."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">Hero · cybernetic-development.png · 2048×2048 · robot concept scene</div>
  <div className="container">
    House robot, not a poster. <b>A human hand and the robot's hand on the same
    ship's wheel</b> — straight from the article's etymology, <i>kybernētēs</i>, steersman.
    Both hands matter: the robot alone is the glass-cockpit failure, the human alone
    is the old world. Engine and Governor in one frame. Replaces the 6KB placeholder
    that currently serves as the social card.
  </div>
</div>
...
<div className="center-small-image">
  <BlogImage
    ...
    className="full"
    alt="Title card: Cybernetic Development."
  />
</div>
...
{/* VISUAL — Engine / Governor loop · HTML blockDiagram · no asset ---------
    THE diagram of this article. Everything downstream is an elaboration of
    it, and right now the central metaphor is carried entirely by prose.
    Build in HTML (preferred house style, cf. how-ai-agents-do-things.mdx)
    using blockDiagram + container + row, so it themes with the site and
    needs no image pipeline.

    STRUCTURE:
      Left box  — ENGINE (Energy): LLM generation. Fast, prolific, tireless,
                  no opinion about whether the output is correct.
      Right box — GOVERNOR (Information): specs, tests, CI gates, telemetry.
                  Slow, opinionated, measures against intent.
      Arrow left->right labelled "output", arrow right->left labelled
      "correction" so the loop visibly closes. The closed loop IS the point.

    Optionally show the pathology as a second, greyed row: an Engine with no
    return arrow, labelled "vibe coding" -- an open loop.

    Keep it two boxes and two arrows. The temptation is to add the whole
    SDLC; resist it. This diagram earns its place by being the simplest
    possible statement of the thesis.

    Alt: "The Engine generates; the Governor measures the output against
    intent and feeds corrections back. A closed loop."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">HTML blockDiagram · Engine / Governor loop · no asset</div>
  <div className="container">
    Two boxes and two arrows. <b>Engine</b> (LLM generation — fast, prolific, no
    opinion about correctness) sends <i>output</i> to <b>Governor</b> (specs, tests,
    CI gates, telemetry), which sends <i>correction</i> back. Optional greyed row
    below shows the pathology: an engine with no return arrow, labelled “vibe
    coding.” The closed loop is the whole thesis — keep it to two boxes.
  </div>
</div>
...
{/* VISUAL — System 1 / System 2 externalized · HTML · no asset ----------
    A mapping table rendered as two side-by-side panels, using the row +
    container vocabulary. Cheap to build, no asset, and it does real work:
    it makes the Kahneman parallel legible at a glance instead of asking the
    reader to hold four terms in their head at once.

    STRUCTURE — two columns, four aligned rows:

      IN THE BRAIN            EXTERNALIZED IN SOFTWARE
      System 1                Vibe Coding (LLMs)
      fast, intuitive          raw generative power
      pattern matching         "vibes", no verification
      -> the ENGINE            -> the ENGINE

      System 2                Engineering Discipline (BDD/TDD)
      slow, deliberate         specs, tests, CI gates
      logical, effortful       constraints and verification
      -> the GOVERNOR          -> the GOVERNOR

    Tint the System 1 / Vibe Coding column and the System 2 / Discipline
    column differently, and reuse the same two tints wherever Engine and
    Governor appear elsewhere on the page so the colour coding carries.

    Alt: "System 1 maps to vibe coding and the engine; System 2 maps to
    engineering discipline and the governor."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">HTML two-panel · System 1 / System 2 mapping · no asset</div>
  <div className="container">
    Two columns — <b>in the brain</b> vs <b>externalized in software</b> — with aligned
    rows: System 1 → vibe coding → the Engine; System 2 → engineering discipline →
    the Governor. Tint the two columns and reuse those same tints for Engine and
    Governor elsewhere on the page so the colour coding carries across diagrams.
  </div>
</div>
...
{/* VISUAL — Five governors as Swiss cheese · HTML · no asset ------------
    Reason's Swiss Cheese model, but populated with THIS article's five
    layers rather than the generic textbook version. That specificity is the
    reason to draw it: the reader can see which layer would have caught
    their last incident.

    STRUCTURE — five stacked horizontal slices, each a container, in order:
      Tool Governors      sandboxes, least privilege, explicit tool access
      Merge Governors     lint, types, unit + integration tests, security
      Release Governors   staging, canary, feature flags, auto-rollback
      Runtime Governors   rate limits, timeouts, breakers, kill switches
      Learning Governors  blameless postmortems feeding new specs + tests

    Punch 2-3 gaps per slice at DIFFERENT horizontal offsets. Draw one
    arrow that threads cleanly through every gap and exits the bottom --
    that is the incident nobody caught, and it is the only part of the
    picture that argues for layering.

    If the offset-holes rendering gets fiddly in pure CSS, an SVG is a fine
    fallback; it is a static figure either way. Do NOT reach for a stock
    Swiss-cheese illustration -- the labels are the value.

    Alt: "Five stacked layers of governors with misaligned gaps, and a
    single failure path threading through all five."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">HTML or SVG · five-governors Swiss cheese · no asset</div>
  <div className="container">
    Reason's model populated with <b>this article's five layers</b> — tool, merge,
    release, runtime, learning — rather than the generic textbook version. Gaps at
    staggered offsets, plus one arrow threading through all five: the incident
    nobody caught. The labels are the value; a stock cheese illustration would not
    earn its place.
  </div>
</div>
...
{/* VISUAL — The abstraction spectrum · HTML · no asset ------------------
    The three tiers the section just walked through, as one figure. Reuse
    the technologyStack container vocabulary from how-ai-agents-do-things.

    STRUCTURE — three stacked tiers, most abstracted at the top:

      PROMPT-TO-APP     Lovable, Bolt.new
                        human-legible: high | agent-manipulable: low
      VISUAL WORKFLOW   N8N
                        human-legible: high | agent-manipulable: medium
      AGENTS AS CODE    Python / TypeScript / Go, Tactus
                        human-legible: medium | agent-manipulable: high

    Two thin gauges per tier -- "human-legible" and "agent-manipulable" --
    running in opposite directions down the stack. That inversion IS the
    abstraction tax, and it sets up the section heading that follows.

    Keep the vendor names light; they will age. The tiers will not.

    Alt: "Three tiers of abstraction. Moving down from prompt-to-app to
    agents-as-code, human legibility falls and agent manipulability rises."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">HTML technologyStack · abstraction spectrum · no asset</div>
  <div className="container">
    Three tiers — <b>prompt-to-app → visual workflow → agents as code</b> — each with
    two thin gauges, <i>human-legible</i> and <i>agent-manipulable</i>, running in
    opposite directions down the stack. That inversion is the abstraction tax, and
    it sets up the heading that follows. Keep vendor names light; they age, the
    tiers don't.
  </div>
</div>
...
{/* VISUAL — Pair programming vs. delegation · HTML · no asset -----------
    Two topologies side by side. This is the structural argument for why
    10x -> 100x requires a task system, and it is much faster to see than
    to read.

    LEFT — "Pair programming" (1:1)
      one human, one agent, a thick two-way arrow between them.
      Caption: every unit of work costs a unit of your attention.

    RIGHT — "Delegation" (1:N)
      one human -> a TASK SYSTEM box (issues in the repo) -> four or five
      agents fanning out. Thin arrows from each agent back INTO the task
      system, not back to the human. That asymmetry is the whole point:
      the task system absorbs the coordination the human used to hold.

    Draw the human the same size in both panels. The temptation is to make
    the right-hand human bigger to suggest leverage; don't -- the human is
    the constant, the fan-out is the variable.

    Alt: "Left, one human paired with one agent. Right, one human directing
    a task system that fans out to several agents, which report back into
    the task system rather than to the human."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">HTML two-panel · pair programming vs. delegation · no asset</div>
  <div className="container">
    Left: <b>1:1</b>, one human and one agent, thick two-way arrow — every unit of
    work costs a unit of attention. Right: <b>1:N</b>, human → task system → several
    agents, with agents reporting back <i>into the task system</i>, not to the human.
    That asymmetry is the argument. Keep the human the same size in both panels.
  </div>
</div>
...
{/* VISUAL — verification-debt.png · matplotlib · 1200x630 --------------
    The one place in this article where a plotted chart beats a diagram,
    because the argument is explicitly about two rates diverging.

    House chart style: matplotlib defaults, white ground, clear axis labels,
    legend, red dashed annotation for the "here's the point" marker.
    (cf. turn-detection-threshold-optimization.png, images/classifiers/*)

      x = time / project lifetime
      y = units of code, LOG scale

      Curve A "Generation capacity"  -- steep exponential. Cheap tokens.
      Curve B "Manual verification"  -- near-flat linear. One human reading.
      Curve C "Automated verification (BDD/TDD + runtime feedback)"
                                     -- tracks A closely, slightly below.

      Shade A-minus-B and label it "verification debt". Annotate the point
      where A crosses B as "you are now shipping code nobody has read."

    The figure has to make ONE thing obvious: the gap is not a constant, it
    compounds. Curve C is the resolution -- it is the only line that keeps
    pace, and it is the cybernetic loop.

    ILLUSTRATIVE -- say so on the figure, as break-even-crossing.png does.
    These are rates and shapes, not measured project data.

    Alt: "Generation capacity rises exponentially while manual verification
    stays flat; the widening gap is verification debt. Automated
    verification is the only line that keeps pace."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">matplotlib · verification-debt.png · 1200×630</div>
  <div className="container">
    Log-scale. <b>Generation capacity</b> climbs exponentially; <b>manual verification</b>
    stays flat; the shaded gap between them is <b>verification debt</b>, annotated
    where it crosses into “you are now shipping code nobody has read.” A third
    curve — automated verification — is the only line that keeps pace, which is the
    resolution. Mark it <i>illustrative</i> on the figure, as the break-even chart does.
  </div>
</div>
