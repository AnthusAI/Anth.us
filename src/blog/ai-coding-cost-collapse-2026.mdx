...
{/* VISUAL SPEC — the fast tier inverted (HTML comparisonChart, no asset) ---
    Use the existing .comparisonChart CSS (see how-ai-agents-do-things.mdx
    for a working example). Renders as real text: accessible, searchable,
    nothing to generate.

    LEFT  — "2025: fast meant cheap"
      Haiku / Flash / mini
      Smaller, distilled, discount tier
      Priced BELOW the flagship
    RIGHT — "2026: fast means premium"
      Fable 5 — $10 / $50 per 1M
      Opus 5 — $5 / $25 per 1M
      GPT-5.6 Sol — $5 / $30, Ultrafast at 14x for a premium
      Priced ABOVE the flagship
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">comparisonChart · the fast tier inverted · no asset needed</div>
  <div className="container">
    Two-column: <b>2025</b> — “fast” meant Haiku/Flash/mini, priced <i>below</i> the
    flagship. <b>2026</b> — “fast” means Fable 5 at $10/$50 against Opus 5’s $5/$25,
    priced <i>above</i> it. Reuses the existing <code>.comparisonChart</code> CSS.
  </div>
</div>
...
Two details matter. First, the Cursor Router—the system that classifies each request and picks a model for it—runs on **Teams and Enterprise plans**. Individual Pro users do not get it. Second, Auto Balance and Auto Intelligence are billed at the selected model's actual API rates, plus a Cursor Token Rate of $0.25 per million tokens whenever it picks a third-party model. Only Auto Cost charges a flat rate regardless of what it selects.<Citation
  data={{
    type: "webpage",
...
<div className="blockDiagram visualPlaceholder">
  <div className="title">matplotlib · price-of-fixed-capability.png · 1200×630</div>
  <div className="container">
    Log-scale decline of the cost to hit a <i>fixed</i> benchmark level, 2023→2026,
    with GPQA / AIME / SWE-bench Verified series and the ~4.7×/yr SWE-bench slope
    annotated. Marker at Feb 2025 for “vibe coding.”
    <br /><b>Caption:</b> Same capability, falling price. The y-axis is logarithmic.
  </div>
</div>
...
{/* VISUAL PROMPT — ai-coding-cost-collapse-2026.png (HERO POSTER) -------
    Replaces the current plain typographic placeholder card.
    Style-match: src/blog/images/the-revolution-will-be-commodified.png
    Square, 2048x2048.
    ...
    a slight diagonal, tightly packed, filling the frame edge to edge.
    High contrast, grungy overspray at the letter edges, subtle scuffs,
    dust, and a soft diagonal highlight across the surface. Square format.
    No imagery other than the text. Text reads:
    "THE $200 HABIT NOW COSTS $20"

    Alt: "Poster: The $200 habit now costs $20."
--------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">Hero poster · ai-coding-cost-collapse-2026.png · 2048×2048</div>
  <div className="container">
    Magenta stencil propaganda poster reading <b>“THE $200 HABIT NOW COSTS $20.”</b>
    Style-matched to <i>The Revolution Will Be Commodified</i>. Replaces the current
    plain placeholder card and becomes the social preview image.
  </div>
</div>
...
<div className="blockDiagram visualPlaceholder">
  <div className="title">matplotlib · two-multiplying-curves.png · 1200×900</div>
  <div className="container">
    Twin stacked panels on a shared time axis: price of fixed capability falling
    (5–10×/yr) above, METR task-horizon doubling every ~7 months below. Optional
    inset showing the product of the two.
    <br /><b>Caption:</b> Two exponentials, same product cycle. They multiply.
  </div>
</div>
...
{/* VISUAL SPEC — the pricey context snowball (HTML blockDiagram) ---------
    No asset needed. Use .blockDiagram + .container, following the pattern
    in how-ai-agents-do-things.mdx.

    Stacked rows, each a turn, with a proportional bar of re-sent context:
      turn 1    |=                          |  small
      turn 10   |==========                 |
      turn 25   |========================   |
      turn 50   |===========================|  full 200k window
    Then a summary row: input:output tokens on a 50-turn session = 25:1

    Bars can be plain characters or nested divs with percentage widths --
    either renders as real text and needs no image generation.

    The point: the agent re-sends a GROWING context on EVERY step, so cost
    tracks loop length, not answer length.
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">blockDiagram · the pricey context snowball · no asset needed</div>
  <div className="container">
    Stacked turn rows (1 / 10 / 25 / 50) with proportional bars for the context
    re-sent each step, closing on the 25:1 input-to-output ratio. Pure HTML/CSS,
    reusing the existing <code>.blockDiagram</code> styling.
  </div>
</div>
...
{/* VISUAL PROMPT — robot-workstation-army.png (photoreal, generated) -----
    The one piece of cinematic art in this article. Lands on the line above:
    the $20 that used to buy one assistant now buys a small team of them.
    Register: the-simulation-told-them-they-would-win.png -- cinematic,
    desaturated, cool-toned, shallow depth of field. 3:2 or square.

    PROMPT:
    Wide cinematic photograph of a vast dim open-plan office that reads like
    a server hall. Endless identical rows of humanoid industrial robots
    seated at cheap matching developer workstations, each robot tethered to
    its desk by a thick braided cable. Monitors glow with dense code. Cool
    teal and green light, deep shadows, shallow depth of field falling off
    into darkness at the back of the frame. Industrial scale, quietly
    dystopian, no visible humans. Photorealistic, 35mm, slight film grain.

    Note the deliberate tonal edge: this section is celebrating cheap
    parallel agents, and the image is faintly unsettling. That tension is
    the point -- it sets up the Jevons catch a few sections later.

    Alt: "Endless rows of identical robots tethered to developer
          workstations in a dim industrial office."
    Caption: "The same budget, a different unit of work."
---------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">photoreal · robot-workstation-army.png · 3:2 or square</div>
  <div className="container">
    Endless rows of humanoid robots tethered by cable to identical developer
    workstations in a dim, teal-lit industrial office. Cinematic, no humans,
    quietly dystopian — the unease is deliberate, setting up the Jevons catch.
    <br /><b>Caption:</b> The same budget, a different unit of work.
  </div>
</div>
