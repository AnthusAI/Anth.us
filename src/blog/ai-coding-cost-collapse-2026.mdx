...
{/* VISUAL — ai-coding-cost-collapse-2026.png (HERO: SIGNATURE CHART) ---
    NOT a stencil poster. This article's argument IS a data trend -- the
    price of a fixed capability level falling off a cliff -- so the chart is
    the most honest and most informative thing that can sit in a feed. A
    slogan tells a scroller nothing; a falling log-scale curve tells them
    the entire thesis before they click.

    SOURCE: scripts/generate-price-of-fixed-capability.py already produces
    exactly this figure. Do not re-derive it. Render a HERO TREATMENT of the
    same data:
      - 1200x630 (social card ratio, no square version needed)
      - title baked into the image, larger than the in-article version,
        since the card is often seen without the headline beside it
      - drop the in-article axis detail that will not survive thumbnailing;
        keep the shape, the log scale, and one annotated number
      - keep the "same capability, falling price" framing explicit

    The in-article copy of the chart stays where it is. This is a second
    render at social-card proportions, not a move.

    Alt: "The cost of reaching a fixed benchmark score falling roughly five
    to ten times per year from 2023 to 2026, on a log scale."
--------------------------------------------------------------------- */}

<div className="blockDiagram visualPlaceholder">
  <div className="title">Hero · ai-coding-cost-collapse-2026.png · 1200×630 · signature chart</div>
  <div className="container">
    Chart, not poster — this article's argument <i>is</i> the trend, so the falling
    log-scale curve states the thesis before anyone clicks. Hero treatment of
    <b>price-of-fixed-capability</b>, which <code>scripts/generate-price-of-fixed-capability.py</code>
    already produces: title baked in, axis detail thinned so it survives
    thumbnailing, one annotated number. The in-article copy stays put.
  </div>
</div>
