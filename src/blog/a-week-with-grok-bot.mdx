...
Disadvantage: the cloud computer can crash. Then the bot cannot do anything. Completely halted. There is a recover option. AWS terms: Recovery Time Objective (RTO) and Recovery Point Objective (RPO). Recovery Time Objective (RTO) here: on the order of five minutes or more (written while waiting; already at least five minutes). Recovery Point Objective (RPO): session messages were gone after restore. Files and artifacts on the computer were still there. Chat-only record can vanish on an unexpected reboot. The disk did not.
