...
* [Structuring Data](#structuring-data) — Using LangChain output parsers to extract structured data from datasets.  In this example: Quickly searching a large database of raw recipe text, looking for something we can make with the one avocado that we have.
...
We need a retry loop as a safeguard layer.  After we call the LLM the second time, to reformat the answer, we can check to make sure we have the right format.  If not, then we can loop back and ask the second LLM a follow-up question.
