...
excerpt: "Fast mode is a latency product, and agent work is almost never latency-bound. You end up buying speed you do not need with tokens you do. Here is the reasoning, the current multipliers, and the specific defaults that switch it on without asking."
...
<BlogImage
  ...
  className="centered"
  alt="Placeholder title card reading Never Use Fast."
/>
...
Start with the button. Every major vendor now sells a Fast variant of its models, and in at least one popular tool it is the default. Turn it off everywhere, then leave it off.

## What Fast actually sells you

Fast is not a better model. It is the same weights on more hardware, or nearer the front of the queue. The only thing it changes is how long you wait.

Priced honestly, too. Anthropic's fast mode runs about 2× the token rate for up to 2.5× the output tokens per second; OpenAI's is the same shape, 2× the rate for up to 2.5× the speed. Read as a raw trade, buying speed slightly cheaper than linear is a good deal.
...
One operational note that costs real money if you skip it: say *in the plan* that implementation runs on Composer 2.5 sub-agents. Say it before you approve. If you click through to implement without that written down, the manager will not put it there on your behalf, and you will get an expensive model doing work a cheap one was going to do. More on the shape of that setup in [A week with Grok Bot](/blog/a-week-with-grok-bot/).
