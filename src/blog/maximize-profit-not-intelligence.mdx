...
Picture this: you're at the helm of a startup, brimming with ideas, ready to leverage the marvels of AI, particularly the genius of GPT 4. But there's a catch - the cost.  You struggle to reach profitability because of astronomical OpenAI bills.  Those output tokens add up!  It's the story of AI applications in 2023.

The progress in AI intelligence is remarkable, but it comes with rising costs. For startups and businesses with innovative ideas, the financial burden of using state-of-the-art models like GPT 4 can be overwhelming. Balancing the cutting-edge capabilities of these models with sustainable, cost-effective solutions is becoming a critical aspect of AI strategy in the business world.

Now, as 2024 unfolds, the narrative shifts from mere intelligence to profitability.  Once you find a use case with business value and prove the concept, you can maximize profit by reducing your costs, by finding the least-sophisticated model that will suit your task.  It's kind of like how you set the optimal price: It's the maximum price that the market will bear.  In AI applications, the path to profitability is to use the <mark>dumbest model that the problem will bear.</mark>
...
Here's where it gets tricky. Although we can quantify similarity, it's not initially clear what level of similarity indicates a sentence is indeed about Spanish immigration law.  What's the threshold of similarity where you say, "Yes, <i>that</i> sentence is about Spanish immigration law"?

What you need is some way to do some kind of statistical analysis so that you know that threshold.  And that's <mark>exactly what machine learning is for!</mark>
...
We <mark>generated a list of 1,500 fake sentences using GPT 3.5</mark>, where each is labeled yes or no: Does the sentence represent a question about Spanish immigration law?  We asked for sentences in English, Spanish and Spanglish.  You can see those in the [`data/labeled.csv`](https://github.com/Anth-us/semantic-text-classification/blob/main/data/labeled.csv) file in the GitHub repository for the experiment.
...
Once we had three different files containing the labeled sentences with the embeddings appended, we split each file into a training and an evaluation data set.  We trained a simple machine learning model as a classifier based on that data.
...
After training the machine-learning models, then we evaluated all three classifiers with our evaluation metrics.
...
This is a big deal because if you use BERT instead of Ada 2 then you can create a classifier with equivalent performance that <mark>doesn't cost any OpenAI API fees</mark> to operate.  If you had a working business use case based on the classifier that depends on AI, then you could make it cheaper by eliminating the OpenAI costs, with no loss in performance.  (Your information security people will appreciate that also.)

## 2024: The Year of Profitable AI

The way forward in AI isn't just about intellectual prowess. It's about smart application. For startups and established businesses alike, the goal is to align AI investments with financial prudence.  We're all excited about artificial intelligence, but our True North in business is profitability.

As we move into 2024, the focus shifts to maximizing business value. It's about leveraging AI not just for its intelligence but for its <mark>contribution to the bottom line.</mark> It's an era where the economic viability of AI solutions becomes as crucial as their technical brilliance.  Always remember that <mark>your goal in business is to maximize profit</mark>, not intelligence.
