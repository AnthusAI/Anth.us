...
Training data is increasingly important, and a lot of it lately is fake.  We use AI to generate training data for AI models.  The training data needs to be realistic, right?

<BlogImage images={props.pageContext.frontmatter.images} index={18} className="full" alt="A handwritten general ledger." />

Researchers at Columbia University confirmed that if you train a handwriting recognition model using numbers that correspond to Benford's Law's predictions, then the model will perform better at recognizing real-world handwritten financial numbers.<Citation
  data={{
    type: "webpage",
