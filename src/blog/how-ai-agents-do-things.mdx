...
In this diagram, an existing system now supports a new AI agent user interface.  The AI agent code runs separately from the business system and integrates with it through API calls, database queries, or whatever makes sense.  The large language model (LLM) serves as a front-end to the user by generating the text for the conversation, using function calling to access the existing software system.
